...
public class PropietarioCafeteriaController {
    ...
    @GetMapping("/propietariocafeteria/propio/{id}")
    public ResponseEntity<Long> obtenerIdPropio(@PathVariable Long id) {
        Long propietarioId = iPropietarioCafeteriaService.obtenerIdPropio(id);
        return ResponseEntity.ok(propietarioId);
    }
}
